...
class Browse extends React.Component {
  ...
  render = () => {
    const { loading, directory } = this.props;
    ...
    if (directory.subdirectories) {
      for (const subdirectory of directory.subdirectories) {
        switch (subdirectory.name) {
          ...
          case "Tidal":
            subdirectory.icons = ["assets/backgrounds/browse-folders.jpgg"];
            break;
...
const mapStateToProps = (state) => {
  const {
    mopidy: { directory: _directory = {} },
    ui: { library_directory_view: view },
  } = state;
